...
// .wrangler/tmp/pages-8EFWJE/functionsWorker-0.2658188351588844.mjs
...
function parseFlight(icao, data) {
  if (!Array.isArray(data) || data.length < 16) return null;
  const lat = data[0];
  const lon = data[1];
  if (typeof lat !== "number" || typeof lon !== "number") return null;
  if (lat === 0 && lon === 0) return null;
  const origin = data[10] || null;
  const dest = data[11] || null;
  return {
    callsign: data[15] || icao,
    lat,
    lon,
    speed: data[4] || 0,
    extra_info: {
...
__name2(parseFlight, "parseFlight");
...
function filterByRadius(flights, homeLat, homeLon, maxKm) {
  return flights.filter((f) => haversineKm(homeLat, homeLon, f.lat, f.lon) <= maxKm);
}
__name(filterByRadius, "filterByRadius");
__name2(filterByRadius, "filterByRadius");
var FR24_URL = "https://data-live.flightradar24.com/zones/fcgi/feed.js?bounds=51.80,51.20,-0.70,0.35&faa=1&satellite=1&mlat=1&flarm=1&adsb=1&gnd=1&air=1&vehicles=1&estimated=1&maxage=14400&gliders=1&stats=1";
var CACHE_KEY = new Request("https://fr24-london-bbox-cache/v1");
var CACHE_TTL = 10;
var MAX_DISTANCE_KM = 5;
async function onRequest(context22) {
  ...
  const parsed = Object.entries(raw).map(([icao, data]) => parseFlight(icao, data)).filter((f) => f !== null);
...
// .wrangler/tmp/bundle-Uil1of/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Uil1of/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2658188351588844.js.map
